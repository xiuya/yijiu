...
export const  UploaderPost= (function(dom,win) {
    ...
    Upload.prototype = {
        ...
        init: function() {
            ...
            // var progress='<progress id="uploadprogress" min="0" max="100" value="0">0</progress>';
        ...
        startUpload: function() {
            ...
            // console.log("正在上传的文件2：", formData, this.fileList,xhr)
              // 获取上传进度
            //   var indexprogress=_this.filterFile.length;
                    　　　　　
            // xhr.upload.onprogress = function (event) { console.log(_this.filterFile.length,event)
            // };
                            ...
                            if (!_this.config.isRepeat) {
                                ...
                                for (var j = 0; j < _this.filterFile.length; j++) {
                                    ...
                                    if (name == _self[i].name) {
                                        setTimeout(()=>{mui.hideLoading()},2000);
                                        isRepeat = true;
                                        mui.toast("请不要选择重复文件！");
                                        break;
                                    }
    ...
    // return function Uploader(id, opt) {
    //     return new Upload(id, opt);
    // }
}
    return Upload;
})(document, window)
...
export const Loading=(function($, window) {
    //显示加载框
